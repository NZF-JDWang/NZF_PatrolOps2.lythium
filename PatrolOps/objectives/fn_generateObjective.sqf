...
private _patrol = missionNamespace getvariable "firstPatrolLocationData"; // Retrieve patrol location data
_patrol params ["_locName","_locPosition","_locRadA","_locRadB","_locDir"]; // Define parameters for patrol location

switch (_obj1) do { // Switch statement based on selected mission objective

	case 4 : { // Case for objective 4
				[_locName, _locPosition] spawn PatrolOps_fnc_manHunt; // Spawn function for capturing an HVI
	...
	case 3 : { // Case for objective 3
				[_locName, _locPosition] call PatrolOps_fnc_weaponsCache; // Call function for destroying an insurgent weapons cache
	...
	case 2 : { // Case for objective 2
				[_locName, _locPosition] call PatrolOps_fnc_iedFactory; // Call function for destroying an IED factory
	...
	case 1 : { // Case for objective 1
				[_locName, _locPosition] call PatrolOps_fnc_vehicleRecovery; // Call function for recovering a damaged HMMWV
